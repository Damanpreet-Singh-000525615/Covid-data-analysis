...
# Aggregate and sum confirmed cases and deaths on a global level
global_confirmed = confirmed_df.groupby('Country/Region').sum().T.sum()
global_deaths = deaths_df.groupby('Country/Region').sum().T.sum()

# Create a new dataframe for global confirmed cases and deaths
...
# Step 1: Create a dictionary of dataframes for total confirmed cases and deaths for each date
datewise_data = {}
...
saskatchewan_data = datewise_data['12/4/21']  # Replace '12/4/21' with the desired date
...
saskatchewan_data = datewise_data['12/4/21']
...
date_for_bar_chart = '12/4/21'
top_20_countries = datewise_data[date_for_bar_chart].nlargest(20, 'Confirmed')
plt.figure(figsize=(12, 8))
top_20_countries.plot(kind='barh', y='Confirmed', color='skyblue')
